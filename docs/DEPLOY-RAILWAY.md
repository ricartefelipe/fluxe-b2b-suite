...
| **Build correcto, mas frontend staging ainda serve Shop ou 404** | O hostname público não estava ligado ao **serviceInstance** certo (o Railway gera um `*.up.railway.app` por defeito). Corrija com a mutation **`serviceDomainUpdate`** na Public API ou com [`scripts/railway-alias-staging-public-domains.py`](../scripts/railway-alias-staging-public-domains.py) (`RAILWAY_API_TOKEN`). **Não** exige novo build. |
...
- **Domínios públicos (ops/admin staging):** [`scripts/railway-alias-staging-public-domains.py`](../scripts/railway-alias-staging-public-domains.py) — mutation `serviceDomainUpdate` para os domínios configurados no Railway Dashboard.
