...
| **shop-frontend: último deploy `FAILED` (staging)** | O tráfego público pode continuar **200** se o deployment **anterior** foi `SUCCESS` (Railway mantém réplicas antigas até o novo passar). No painel: **Deployments** → abrir o `FAILED` e ver o erro (build Docker, healthcheck `GET /`, timeout). Via CLI: `railway deployment list -s shop-frontend`; `railway logs <DEPLOYMENT_ID> --build` / `--deployment` (se existir build associado). **Causa frequente no monorepo:** push em **`docs/`** ou noutras pastas sem alterar `saas-suite-ui` ainda dispara CI/CD do serviço — o deploy novo pode falhar sem impacto imediato no site. Mitigação: no serviço **shop**, definir **Root Directory** = `saas-suite-ui` (se ainda não estiver) e, no Railway, **Watch Paths** / padrões de ignore para excluir `docs/**` (quando suportado), ou aceitar redeploy só quando mudar código do front. Recuperação: **Redeploy** a partir de um commit que altere o shop com sucesso, ou **rollback** para o último deployment `SUCCESS`. |
