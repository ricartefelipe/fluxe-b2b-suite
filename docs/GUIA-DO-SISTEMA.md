...
| **Detalhe/Edição de Tenant** | Formulário com nome, slug, plano (starter/pro/enterprise), região. Ações: ativar, suspender. | Edite os campos e salve. Suspenda tenants inadimplentes. |
| **Onboarding de Tenant** | Stepper de 5 etapas: **1. Organização** (nome, slug, região), **2. Plano** (com preços e features de cada plano), **3. Configuração** (feature flags e e-mail do admin), **4. Revisão**, **5. Pronto**. | Siga o fluxo passo a passo para criar um novo tenant completo. |
| **Políticas (ABAC/RBAC)** | Tabela de políticas de acesso: permission code, efeito (Permitir/Negar), habilitada, planos e regiões permitidas. Criar, editar, remover. | Gerencie quem pode fazer o quê. Ex: `orders:write` permitido apenas para planos pro/enterprise. |
| **Feature Flags** | Flags por tenant: nome, habilitada, descrição. Criar, toggle, editar, remover. | O tenant do usuário é auto-selecionado ao entrar. Troque o tenant no header se necessário. Ative/desative funcionalidades por tenant. |
| **Usuários** | Lista de usuários: nome, e-mail, roles, status. Convidar, editar, remover. | Gerencie o acesso dos usuários ao sistema. |
| **Log de Auditoria** | Tabela de eventos: data, ação, resultado, usuário, recurso, correlation ID. | Investigue ações sensíveis. Filtre por ação ou usuário. |
