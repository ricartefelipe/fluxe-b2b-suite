...
@Component({
  ...
  template: `
    ...
    @if (facade.selectedOrder(); as order) {
      ...
      <div class="details-grid">
        <mat-card>
          <mat-card-header><mat-card-title>{{ i18n.messages().ops.info }}</mat-card-title></mat-card-header>
          <mat-card-content>
            <p><strong>{{ i18n.messages().orders.customer }}:</strong> {{ order.customerId }}</p>
            <p><strong>{{ i18n.messages().payments.currency }}:</strong> {{ order.currency }}</p>
            <p><strong>{{ i18n.messages().common.total }}:</strong> {{ order.currency }} {{ order.totalAmount | number:'1.2-2' }}</p>
            <p><strong>{{ i18n.messages().ops.createdAt }}:</strong> {{ fmtDate(order.createdAt) }}</p>
            <p><strong>{{ i18n.messages().ops.updatedAt }}:</strong> {{ fmtDate(order.updatedAt) }}</p>
            @if (order.correlationId) {
              <p><strong>{{ i18n.messages().admin.correlationId }}:</strong> <code>{{ order.correlationId }}</code></p>
            }
          </mat-card-content>
        </mat-card>

        <mat-card>
          <mat-card-header><mat-card-title>{{ i18n.messages().orders.orderItems }} ({{ order.items.length }})</mat-card-title></mat-card-header>
          <mat-card-content>
            <table mat-table [dataSource]="order.items" class="full-width">
              ...
              <ng-container matColumnDef="unitPrice">
                <th mat-header-cell *matHeaderCellDef>{{ i18n.messages().ops.unitPrice }}</th>
                <td mat-cell *matCellDef="let i">{{ i.unitPrice | number:'1.2-2' }}</td>
              </ng-container>
              <ng-container matColumnDef="subtotal">
                <th mat-header-cell *matHeaderCellDef>{{ i18n.messages().ops.subtotal }}</th>
  ...
  styles: [`
    ...
    .details-grid { display: grid; grid-template-columns: 1fr 1fr; gap: var(--app-space-16, 16px); }
...
export class OrderDetailPage implements OnInit {
  ...
  async confirm(): Promise<void> {
    ...
    if (!order) return;
    const ref = this.dialog.open(ConfirmDialogComponent, { data: { title: this.i18n.messages().ops.orderConfirmTitle, message: this.i18n.messages().ops.orderConfirmMessage } });
    const ok = await firstValueFrom(ref.afterClosed());
    if (!ok) return;
    const o = await this.facade.confirmOrder(order.id);
    if (o) this.snackBar.open(this.i18n.messages().ops.orderConfirmedSnackbar, 'OK', { duration: 2000 });
  }

  async cancel(): Promise<void> {
    const order = this.facade.selectedOrder();
    if (!order) return;
    const ref = this.dialog.open(ConfirmDialogComponent, { data: { title: this.i18n.messages().ops.orderCancelTitle, message: this.i18n.messages().ops.orderCancelMessage, danger: true } });
    const ok = await firstValueFrom(ref.afterClosed());
    if (!ok) return;
    const o = await this.facade.cancelOrder(order.id);
    if (o) this.snackBar.open(this.i18n.messages().ops.orderCancelledSnackbar, 'OK', { duration: 2000 });
  }
