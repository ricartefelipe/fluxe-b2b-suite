...
import { CursorResponse, toParams } from '@saas-suite/shared/http';
...
export class OrdersApiClient {
  ...
  listOrders(p?: OrderListParams): Observable<CursorResponse<Order>> {
    return this.http.get<CursorResponse<Order>>(`${this.base}/v1/orders`, { params: toParams(p as Record<string, unknown>) });
  }
  ...
  listInventory(sku?: string): Observable<CursorResponse<InventoryItem>> {
    return this.http.get<CursorResponse<InventoryItem>>(`${this.base}/v1/inventory`, { params: toParams(sku != null ? { sku } : undefined) });
  }
  ...
  listAdjustments(p?: AdjustmentListParams): Observable<CursorResponse<InventoryAdjustment>> {
    return this.http.get<CursorResponse<InventoryAdjustment>>(`${this.base}/v1/inventory/adjustments`, { params: toParams(p as Record<string, unknown>) });
  }
}
