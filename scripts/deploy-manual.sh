...
# Uso: defina as variáveis abaixo ou crie .env.deploy com VPS_HOST, VPS_USER, VPS_SSH_KEY, GHCR_ORG.
# Para publicar a nova imagem do spring-saas-core: BUILD_CORE=1 ./scripts/deploy-manual.sh
# Requer: docker (e login no GHCR), rsync, ssh.
...
# Carregar .env.deploy se existir (VPS_HOST, VPS_USER, VPS_SSH_KEY, GHCR_ORG)
...
# Opcional: construir e dar push da imagem spring-saas-core a partir do repo local
...
# ---------- 1. (Opcional) Build e push da imagem spring-saas-core ----------
if [[ "$BUILD_CORE" == "1" || "$BUILD_CORE" == "true" ]]; then
  ...
  log "BUILD_CORE não está ativado. Usando imagem já existente no GHCR (latest)."
fi
